...
const formVideo = new FormData();
formVideo.append("_token", document.querySelector("#csrf-token").content);
fetchVideoAPI(document.getElementById('admin-salevideo-upload-url').value, formVideo);

// Validate video uploading
Validator({
    form: '#admin-salevideo-upload',
    rules: [
        Validator.file({
            required: true,
    ...
    onSubmit: (data) => {
        ...
        let videoFile = document.querySelector('#sale_video_input').files[0];
