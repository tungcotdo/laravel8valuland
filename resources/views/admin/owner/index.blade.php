...
<div class="card-btns">
  ...
  <a href="{{route('admin.owner.arrange')}}" class="btn btn-sm btn-warning"><i class="bi bi-arrow-repeat"></i> Sắp xếp</a>
  <a href="{{route('admin.owner.truncate')}}" onclick="return confirm('Bạn có muốn xóa hết dữ liệu không?')" class="btn btn-sm btn-danger"><i class="bi bi-trash"></i> Xóa dữ liệu</a>
</div>
              ...
              <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    @foreach($owners as $key => $value)
                        <tr>
                            <td>{{ $value->code }}</td>
                            <td style="max-width: 200px; overflow-x: auto;">{{ $value->owner_name }}</td>
                            <td style="max-width: 200px; overflow-x: auto;">{{ $value->owner_phone }}</td>
